...
logger = get_logger("order_items_etl")

class OrderItemsETL:
    TEMP_DIR = "data/tmp/order_items"
    ...
    def validate(self, df: pd.DataFrame):
        ...
        valid_product_ids = {r[0] for r in cur.fetchall()}
        cur.close()
        valid = df[df["order_id"].isin(valid_order_ids) & df["product_id"].isin(valid_product_ids)]
    ...
    def upload_invalid(self, file_meta: Dict, df: pd.DataFrame):
        ...
        object_path = f"bad_records/order_items/{os.path.basename(temp_file)}"
